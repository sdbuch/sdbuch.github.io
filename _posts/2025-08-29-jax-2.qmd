...
This storage is actually essential, as well. We can't just rely on intelligent recomputation of
keys and values in general, because *the keys and values at position $i$ and layer
$\ell+1$ of the model depend on the queries at all positions $j < i$ at layer $\ell$*!
So because of the multilayer structure of the transformer, as soon as we try to use
*some* recomputation, we need to recompute *all* previous queries, which is maximally
wasteful.
